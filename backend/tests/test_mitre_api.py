...
async def test_technique_list_supports_tactic_and_search_filters() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get("/api/v1/mitre/techniques")
...
async def test_technique_detail_returns_pinned_subtechnique_metadata() -> None:
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get("/api/v1/mitre/techniques/t1110.003")
        missing = await client.get("/api/v1/mitre/techniques/T9999")
...
async def test_rule_mapping_api_validates_rules_and_leaves_ml_unmapped() -> None:
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        spray = await client.get("/api/v1/mitre/rules/rule_002_password_spray/techniques")
        ml = await client.get("/api/v1/mitre/rules/rule_006_ml_anomaly/techniques")
        missing = await client.get("/api/v1/mitre/rules/rule_missing/techniques")
